...
export default function BlogListingClient({ blogs }: BlogListingClientProps) {
  ...
  const filteredBlogs = useMemo(() => {
    ...
    return blogs.filter((blog) => blog.category?.toUpperCase() === activeCategory);
          ...
          <div className="flex items-center gap-12 overflow-x-auto no-scrollbar">
            {categories.map((cat) => (
              <button
                ...
                className={`whitespace-nowrap text-[11px] font-black tracking-[0.4em] uppercase transition-all relative py-2 ${
                  activeCategory === cat
